...
9. For one-long-source repurposing, add a separate editorial-unit obligation. Freeze an independent deliverable map whose units each contain a complete promise/setup/payoff and source evidence. Then require `1 source hash → N semantic units → N distinct editable projects → N decoded renders → N receipts`. File count cannot define N; a single compilation fallback, duplicated source ranges, missing units, or a recut with an unchanged editorial fingerprint blocks promotion. Prior candidate/Skill outputs may be baselines but cannot label their own ground truth.
